...
from auth import get_user_from_session, setup_auth_middleware
from config import *
...
def generate_pdf_report(inspection: Dict[str, Any]) -> str:
    ...
    for category in inspection["categories"]:
        ...
        if category.get("description"):
            story.append(
                Paragraph(f"Description: {category['description']}", styles["Normal"])
            )
            story.append(Spacer(1, 12))
...
async def list_industries(request: Request) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        "industries.html",
        {"request": request, "user": user, "industries": AVAILABLE_INDUSTRIES},
    )
    ...
    return templates.TemplateResponse(
        "new_industry_inspection.html",
        {
            ...
            "breadcrumbs": [
                ...
                {
                    "name": f"New {AVAILABLE_INDUSTRIES[industry_id]['name']} Inspection",
                    "url": f"/industries/{industry_id}/new",
                },
            ],
        },
    )
...
async def list_inspections(request: Request) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        "redirect_to_guided_inspections.html",
        {
            "request": request,
            "breadcrumbs": [
                {"name": "Home", "url": "/"},
...
async def new_inspection(request: Request) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        "redirect_to_guided_inspection.html", {"request": request}
    )


@app.get("/api/inspections")
async def list_inspections_api() -> Dict[str, Any]:
    """List all inspections."""
    from modules.inspection.service import load_inspections
    ...
    if inspection.vehicle_info:
        ...
        if vehicle_data.get("vin"):
            try:
                ...
                if decoded_vehicle.engine_displacement and not vehicle_data.get(
                    "engine"
                ):
                    vehicle_data["engine"] = decoded_vehicle.engine_displacement
                if decoded_vehicle.transmission_type and not vehicle_data.get(
                    "transmission"
                ):
                ...
                if decoded_vehicle.country_of_origin and not vehicle_data.get(
                    "country_of_origin"
                ):
                    vehicle_data["country_of_origin"] = (
                        decoded_vehicle.country_of_origin
                    )
                if decoded_vehicle.plant_code and not vehicle_data.get("plant_code"):
                    vehicle_data["plant_code"] = decoded_vehicle.plant_code
                if decoded_vehicle.serial_number and not vehicle_data.get(
                    "serial_number"
                ):
    ...
    for category_name, category_data in template["inspection_points"].items():
        ...
        for item_name in category_data.get("items", []):
            category["items"].append(
                {"name": item_name, "grade": "N/A", "notes": "", "photos": []}
            )
...
async def view_inspection(request: Request, inspection_id: str) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "user": user, "inspection": inspection},
    )
...
@app.post("/api/inspections/{inspection_id}")
async def post_inspection(inspection_id: str) -> Dict[str, str]:
    """Handle POST requests to specific inspection IDs."""
    inspection = find_inspection(inspection_id)
    if not inspection:
        raise HTTPException(status_code=404, detail="Inspection not found")
    raise HTTPException(status_code=405, detail="Method not allowed for this endpoint")


@app.put("/api/inspections/{inspection_id}")
async def update_inspection(
    inspection_id: str, inspection: Dict[str, Any]
) -> Dict[str, str]:
    """Update inspection data."""
    ...
    report = {
        ...
        "statistics": {
            ...
            "completion_percentage": (
                (graded_items / total_items * 100) if total_items > 0 else 0
            ),
