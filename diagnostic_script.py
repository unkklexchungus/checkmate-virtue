...
class FastAPIDiagnostic:
    ...
    async def test_route(self, session: aiohttp.ClientSession, method: str, path: str, expected_status: int = 200, description: str = "") -> Dict[str, Any]:
        """Test a single route."""
        ...
        try:
            ...
            async with session.request(method, url, allow_redirects=follow_redirects) as response:
                ...
                if response.status == 200 and response.content_type == "text/html":
                    ...
                    if ("jinja2.exceptions" in content.lower() or 
                        "template error" in content.lower() or 
                        "template exception" in content.lower() or
                        "jinja2 error" in content.lower()):
                        test_result["status"] = "FAIL"
    ...
    async def test_navigation_flow(self, session: aiohttp.ClientSession) -> None:
        ...
        navigation_tests = [
            ...
            },
            {
                ...
                "steps": [
                    ...
                    ("GET", "/inspection/9999999", 404, "Invalid inspection (should 404)")
                ]
            },
            {
                ...
                "steps": [
                    ("GET", "/industries", 200, "Industries page"),
                    ("GET", "/industries/automotive/new", 200, "Automotive new inspection"),
                    ("GET", "/industries/invalid/new", 404, "Invalid industry (should 404)")
                ]
            }
        ]
        ...
        for nav_test in navigation_tests:
            ...
            for method, path, expected_status, description in nav_test["steps"]:
                step_result = await self.test_route(session, method, path, expected_status, description)
                    ...
                    if response.status == 200:
                        ...
                        if "breadcrumb" in content.lower():
                            ...
                            if 'href=' in content and 'breadcrumb' in content:
                                breadcrumb_result["breadcrumb_links"] = ["Breadcrumb links detected"]
    ...
    async def test_error_cases(self, session: aiohttp.ClientSession) -> None:
        ...
        for method, path, expected_status, description in error_tests:
            error_result = await self.test_route(session, method, path, expected_status, description)
    ...
    async def test_api_endpoints(self, session: aiohttp.ClientSession) -> None:
        ...
        api_tests = [
            ...
            ("GET", "/api/industries/automotive/template", 200, "Automotive template API"),
            ("GET", "/vehicle/health", 200, "Vehicle service health"),
            ("GET", "/invoices/api/invoices", 404, "Invoices API (should 404 without auth)"),
        ]
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"Success Rate: {(summary['passed'] / summary['total_routes'] * 100):.1f}%" if summary['total_routes'] > 0 else "N/A")
        report.append("")
        ...
        for test in self.results["route_tests"]:
            ...
            report.append(f"{status_icon} {test['method']} {test['path']} - {test['description']}")
            if test["error"]:
        ...
        for nav_test in self.results["navigation_tests"]:
            ...
            for step in nav_test["steps"]:
                ...
                report.append(f"   {step_icon} {step['method']} {step['path']} - {step['description']}")
                if step["error"]:
        ...
        for breadcrumb_test in self.results["breadcrumb_tests"]:
            ...
            report.append(f"{status_icon} {breadcrumb_test['path']} - {breadcrumb_test['description']}")
        ...
        for template_test in self.results["template_tests"]:
            ...
            report.append(f"{status_icon} {template_test['path']} - {template_test['description']}")
        ...
        for error_test in self.results["error_cases"]:
            ...
            report.append(f"{status_icon} {error_test['method']} {error_test['path']} - {error_test['description']}")
            if error_test["error"]:
        ...
        # Issues Found
        issues = []
        for test in self.results["route_tests"] + self.results["navigation_tests"] + self.results["breadcrumb_tests"] + self.results["template_tests"] + self.results["error_cases"]:
            if test["status"] != "PASS":
...
async def main():
    ...
    print("=" * 80)
    print(f"✅ Report saved to: WE_FIX_WITHOUT_MAKING_ANY_CHANGES_UNTIL_I_APPROVE_THEM.txt")
    print(f"📊 Summary: {results['summary']['passed']}/{results['summary']['total_routes']} tests passed")
    print("=" * 80)
