...
from app.models import InspectionCreate
from .service import (find_inspection, generate_inspection_id,
                      load_inspection_template, save_inspection,
                      update_inspection)
...
async def inspection_form(request: Request):
    ...
    return templates.TemplateResponse(
        "inspection_form.html", {"request": request, "template": template}
    )
...
async def create_inspection(data: InspectionCreate):
    ...
    print(f"Received inspection data: {data}")
    ...
    if data.vin:
        try:
            ...
            # Add timeout to VIN decoding to prevent hanging
            try:
                decoded_vehicle = await asyncio.wait_for(decode_vin(data.vin), timeout=5.0)
                vehicle_info = {
                    "vin": data.vin,
                ...
                print(f"VIN decoding timed out for {data.vin}")
    ...
    # Save to database
    print(f"Attempting to save inspection: {inspection_data['id']}")
    if save_inspection(inspection_data):
        print(f"Inspection saved successfully: {inspection_data['id']}")
        return {
        ...
        print(f"Failed to save inspection: {inspection_data['id']}")
...
async def post_inspection(inspection_id: str):
...
async def upload_photo(
    ...
    for item_data in inspection["items"]:
        if (
            item_data["step"] == step
            and item_data["subcategory"] == subcategory
            and item_data["item"] == item
        ):
