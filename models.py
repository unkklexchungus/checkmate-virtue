...
class InvoiceItem(BaseModel):
    ...
    tax_rate: Decimal = Field(
        default=0, ge=0, le=100, description="Tax rate percentage"
    )
    discount_percent: Decimal = Field(
        default=0, ge=0, le=100, description="Discount percentage"
    )
...
class Invoice(BaseModel):
    ...
    status: InvoiceStatus = Field(
        default=InvoiceStatus.DRAFT, description="Invoice status"
    )
    ...
    discount_amount: Decimal = Field(
        default=0, ge=0, description="Total discount amount"
    )
    ...
    materials_total: Decimal = Field(
        default=0, ge=0, description="Total materials charges"
    )
    ...
    def calculate_totals(self):
        ...
        self.labor_total = sum(
            item.subtotal for item in self.items if item.item_type == ItemType.LABOR
        )
        self.parts_total = sum(
            item.subtotal for item in self.items if item.item_type == ItemType.PARTS
        )
        self.materials_total = sum(
            item.subtotal for item in self.items if item.item_type == ItemType.MATERIALS
        )
        self.service_total = sum(
            item.subtotal for item in self.items if item.item_type == ItemType.SERVICE
        )
...
class InvoiceTemplate(BaseModel):
    ...
    default_jobs: List[Dict[str, Any]] = Field(
        default=[], description="Default jobs/work orders"
    )
    default_items: List[Dict[str, Any]] = Field(
        default=[], description="Default invoice items"
    )
